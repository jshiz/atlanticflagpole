...
export function FinderDrawer({ isOpen, onClose, side = "right" }: FinderDrawerProps) {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      ...
      >
        <FinderContent onClose={onClose} />
      </SheetContent>
    </Sheet>
  )
}

export function FinderContent({ onClose }: { onClose: () => void }) {
  const [height, setHeight] = useState<string>("")
  const [location, setLocation] = useState<"residential" | "commercial" | null>(null)
      ...
      <div className="p-2.5 bg-[#0B1C2C] text-white shrink-0 border-b-2 border-[#C8A55C]/30">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2 text-sm font-bold">
            <Compass className="w-4 h-4 text-[#C8A55C]" />
            Flagpole Finder
          </div>
          <button
            onClick={onClose}
        ...
        <p className="text-[10px] text-gray-300">Find your perfect flagpole</p>
      </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className="flex flex-col items-center gap-2 p-3 rounded-xl border-2 transition-all border-gray-200 hover:border-[#C8A55C]/50"
              >
            ...
            <div className="grid grid-cols-2 gap-2">
              {recommendations.map((rec) => (
                <button
                  ...
                  className="p-3 rounded-xl border-2 text-left transition-all border-gray-200 hover:border-[#C8A55C]/50"
                >
          ...
          <div className="pt-4 border-t border-gray-200">
            ...
            <div className="space-y-2">
              <Link href="/collections/telescoping-flagpoles" onClick={onClose}>
                <Button
                  variant="outline"
                  className="w-full justify-start text-sm border-gray-300 hover:border-[#C8A55C] hover:bg-[#C8A55C]/5 bg-transparent"
                >
