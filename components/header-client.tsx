            ...
            <div className="hidden lg:flex items-center gap-6">
              <div className="relative" onMouseEnter={handleNavEnter} onMouseLeave={handleNavLeave}>
                ...
                {isMegaMenuOpen && activeDropdown && (
                  <div
                    className="absolute left-0 right-0 top-full bg-white border-t-2 border-[#C8A55C] shadow-2xl z-50"
                    style={{
                      minWidth: "800px",
                        ...
                        <div key={item.id} className="container mx-auto px-4 py-8">
                          <div className="grid grid-cols-12 gap-8 max-w-6xl mx-auto">
                            ...
                            <div className="col-span-4">
                              <h3 className="text-lg font-bold text-[#0B1C2C] mb-4 pb-2 border-b border-gray-200">
                                {item.title}
                              </h3>
                              <ul className="space-y-3">
                                {item.items?.map((subItem) => (
                                  <li key={subItem.id}>
                                    <Link
                                      href={subItem.url}
                                      className="text-[#0B1C2C] hover:text-[#C8A55C] transition-all duration-200 text-sm block py-1 font-medium relative group/link cursor-pointer"
                                    >
                                      <span className="relative">
                                        {subItem.title}
                                        <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-[#C8A55C] group-hover/link:w-full transition-all duration-300"></span>
                                      </span>
                                    </Link>
                                  </li>
                                ))}
                              ...
                              <Link
                                href={item.url}
                                className="inline-flex items-center gap-1 mt-6 text-[#C8A55C] hover:text-[#a88947] font-semibold text-sm group/viewall transition-colors duration-200 cursor-pointer"
                              >
                                View All {item.title}
                            ...
                            {/* Right side - Featured products with images */}
                            <div className="col-span-8">
                              ...
                              {collectionData?.products?.nodes ? (
                                <div className="grid grid-cols-3 gap-6">
                                  {collectionData.products.nodes.slice(0, 3).map((product: any) => (
                                    <Link
                                      key={product.id}
                                      href={`/products/${product.handle}`}
                                      className="group/product cursor-pointer"
                                    >
                                      <div className="aspect-square bg-gray-100 rounded-lg overflow-hidden mb-3 shadow-sm group-hover/product:shadow-md transition-shadow duration-300">
                                        {product.featuredImage ? (
                                          <Image
                                            ...
                                            width={300}
                                            height={300}
                                            className="w-full h-full object-cover group-hover/product:scale-110 transition-transform duration-500 ease-out"
                                          />
                                        ) : (
                                      ...
                                      <h4 className="text-sm font-medium text-[#0B1C2C] group-hover/product:text-[#C8A55C] transition-colors duration-200 line-clamp-2 mb-1">
                                        {product.title}
                                      </h4>
                                ...
                                <div className="grid grid-cols-3 gap-6">
                                  {item.items?.slice(0, 3).map((subItem) => (
                                    <Link key={subItem.id} href={subItem.url} className="group/product cursor-pointer">
                                      <div className="aspect-square bg-gradient-to-br from-gray-100 to-gray-200 rounded-lg overflow-hidden mb-3 flex items-center justify-center shadow-sm group-hover/product:shadow-md transition-shadow duration-300">
                                        <span className="text-5xl">🏴</span>
                                      </div>
                                      <h4 className="text-sm font-medium text-[#0B1C2C] group-hover/product:text-[#C8A55C] transition-colors duration-200">
                                        {subItem.title}
                                      </h4>
                                    </Link>
                                  ))}
