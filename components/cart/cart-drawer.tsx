...
export function CartContent({ onClose }: CartContentProps) {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-white">
      {/* Header */}
      <div className="p-2.5 bg-[#0B1C2C] text-white border-b-2 border-[#C8A55C]/30 shrink-0">
        ...
        {itemCount > 0 && (
          <Link href="/cart" onClick={onClose}>
            <Button className="w-full bg-[#C8A55C] hover:bg-[#B69446] text-white font-bold mt-2 h-8 text-xs shadow-lg rounded-xl">
        ...
        {itemCount === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center py-6">
            ...
            <Link href="/products" onClick={onClose}>
              <Button className="bg-[#C8A55C] hover:bg-[#B69446] text-white px-6 text-xs rounded-xl">Shop Now</Button>
            </Link>
          </div>
        ) : (
      ...
      {/* Footer */}
      {itemCount > 0 && (
        <div className="border-t-2 border-[#C8A55C]/30 p-2.5 bg-[#0B1C2C] shrink-0">
          <div className="flex justify-between items-center mb-2">
            <span className="text-xs font-medium text-gray-300">Subtotal:</span>
            <span className="text-lg font-bold text-[#C8A55C]">${Number(subtotal).toFixed(2)}</span>
          </div>

          <Link href="/cart" onClick={onClose}>
            <Button className="w-full bg-[#C8A55C] hover:bg-[#B69446] text-white font-bold py-2.5 text-sm shadow-lg rounded-xl">
