...
export function CartContent({ onClose }: CartContentProps) {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-white">
      {/* Header */}
      <div className="p-3 bg-[#0B1C2C] text-white border-b-2 border-[#C8A55C]/30 shrink-0">
        <div className="flex items-center justify-between">
          <div className="text-white flex items-center gap-2 text-sm font-bold">
            <ShoppingCart className="w-4 h-4 text-[#C8A55C]" />
            Cart ({itemCount})
          </div>
          <button 
            onClick={onClose} 
        ...
        {itemCount > 0 && (
          <Link href="/cart" onClick={onClose}>
            <Button className="w-full bg-[#C8A55C] hover:bg-[#B69446] text-white font-bold mt-2 h-9 text-xs shadow-lg rounded-xl">
      ...
      {/* Content */}
      <div className="flex-1 overflow-y-auto p-3 bg-white overscroll-contain touch-pan-y">
        {itemCount === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center py-6">
            <div className="w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center mb-3">
              <ShoppingCart className="w-8 h-8 text-gray-400" />
            </div>
            <h3 className="text-base font-bold text-[#0B1C2C] mb-1">Your cart is empty</h3>
            <p className="text-xs text-gray-500 mb-4">Add some flagpoles to get started</p>
            <Link href="/products" onClick={onClose}>
              <Button className="bg-[#C8A55C] hover:bg-[#B69446] text-white px-6 text-xs rounded-xl">
                Shop Now
              </Button>
            </Link>
          </div>
                  ...
                  <div className="flex items-center justify-between gap-2 mt-auto">
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="w-7 h-7 rounded-lg border border-[#0B1C2C] hover:bg-[#0B1C2C] hover:text-white flex items-center justify-center transition-colors"
                      >
                        <Minus className="w-3 h-3" />
                      </button>
                      <span className="text-xs font-bold text-[#0B1C2C] w-6 text-center">{line.quantity}</span>
                      <button
                        onClick={() => updateLineQuantity(line.id, line.quantity + 1)}
                        className="w-7 h-7 rounded-lg border border-[#0B1C2C] hover:bg-[#0B1C2C] hover:text-white flex items-center justify-center transition-colors"
                      >
      ...
      {/* Footer */}
      {itemCount > 0 && (
        <div className="border-t-2 border-[#C8A55C]/30 p-3 bg-[#0B1C2C] shrink-0">
          <div className="flex justify-between items-center mb-3">
            <span className="text-xs font-medium text-gray-300">Subtotal:</span>
            <span className="text-xl font-bold text-[#C8A55C]">${Number(subtotal).toFixed(2)}</span>
          </div>
          
          <Link href="/cart" onClick={onClose}>
            <Button className="w-full bg-[#C8A55C] hover:bg-[#B69446] text-white font-bold py-3 text-sm shadow-lg rounded-xl">
